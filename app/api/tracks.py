...
from .schemas import TracksPage, TopArtist, Summary
...
def list_tracks(
    ...
    if tempo_min is not None and tempo_max is not None and tempo_min > tempo_max:
        raise HTTPException(status_code=422, detail="tempo_min cannot be greater than tempo_max")
    return get_tracks(
        limit=limit, offset=offset, q=q, artist=artist,
        min_danceability=min_danceability, tempo_min=tempo_min, tempo_max=tempo_max,
        sort=sort, order=order
    )

@router.get("/stats/top-artists", response_model=List[TopArtist])
def top_artists(limit: int = Query(10, ge=1, le=100)):
    return get_top_artists(limit=limit)

@router.get("/stats/summary", response_model=Summary)
def summary():
    return get_summary()
